    ...
    <div className="min-h-screen bg-white">
      ...
      <nav className="border-b border-gray-100 sticky top-0 bg-white/80 backdrop-blur z-50">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/login">
              <Button size="sm" className="bg-violet-600 hover:bg-violet-700">Empezar gratis</Button>
            </Link>
          </div>
        </div>
      </nav>
      ...
      <section className="max-w-6xl mx-auto px-4 pt-20 pb-24 text-center">
        ...
        <h1 className="text-5xl md:text-6xl font-bold text-gray-900 mb-6 leading-tight tracking-tight">
          ...
          <span className="text-violet-600">convierten clientes</span>
        </h1>
        ...
        <div className="flex items-center justify-center gap-4 flex-wrap">
          <Link href="/login">
            <Button size="lg" className="bg-violet-600 hover:bg-violet-700 h-12 px-8 text-base">
              Empezar gratis
      ...
      <section className="border-y border-gray-100 bg-gray-50 py-10">
        <div className="max-w-4xl mx-auto px-4 grid grid-cols-3 gap-8 text-center">
          {[
            ...
            <div key={s.label}>
              <p className="text-3xl font-bold text-violet-600 mb-1">{s.number}</p>
      ...
      <section className="bg-violet-50 py-24">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              { icon: <Video className="w-5 h-5 text-violet-600" />, title: "Video testimonials", desc: "Tus clientes graban videos directamente desde el navegador. Sin apps externas." },
              { icon: <Star className="w-5 h-5 text-violet-600" />, title: "Sistema de ratings", desc: "Recolectá puntuaciones de 1 a 5 estrellas junto con cada testimonio." },
              { icon: <CheckCircle className="w-5 h-5 text-violet-600" />, title: "Aprobación manual", desc: "Vos decidís qué testimonios aparecen públicamente. 100% control editorial." },
              { icon: <Code className="w-5 h-5 text-violet-600" />, title: "Widget embed", desc: "Código para pegar en cualquier web. Compatible con Shopify, WooCommerce, Webflow y más." },
              { icon: <BarChart2 className="w-5 h-5 text-violet-600" />, title: "Múltiples espacios", desc: "Creá un espacio por producto, cliente o campaña. Sin límite de espacios." },
              { icon: <Shield className="w-5 h-5 text-violet-600" />, title: "Sin contraseñas", desc: "Login por magic link. Tus clientes entran al formulario sin crear cuenta." },
            ].map((f) => (
              <div key={f.title} className="bg-white rounded-xl p-6 border border-violet-100">
      ...
      <section className="max-w-6xl mx-auto px-4 py-24">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">
          ...
          <div className="bg-violet-600 rounded-2xl p-8 relative overflow-hidden">
            ...
            <p className="text-sm text-violet-200 mb-6">Para profesionales</p>
            <p className="text-4xl font-bold text-white mb-6">$19<span className="text-base font-normal text-violet-300">/mes</span></p>
            <ul className="space-y-3 mb-8">
              {["Testimonios ilimitados", "Sin branding de TestiFlow", "Widget embed premium", "Soporte prioritario"].map(f => (
                <li key={f} className="flex items-center gap-2 text-sm text-white">
                  <CheckCircle className="w-4 h-4 text-violet-300 shrink-0" />{f}
                </li>
              ))}
            </ul>
            <Link href="/login">
              <Button className="w-full bg-white text-violet-600 hover:bg-violet-50">Empezar con Pro</Button>
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="bg-violet-600 py-20">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <Link href="/login">
            <Button size="lg" className="bg-white text-violet-600 hover:bg-violet-50 h-12 px-10 text-base font-semibold">
              Crear cuenta gratis
              <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          </Link>
